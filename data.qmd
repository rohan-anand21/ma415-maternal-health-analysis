...
> natality_csv <- read_csv(here::here("dataset-ignore", 
+     "nat2021us.csv"), n_max = 1e+05, show_col_types = FALSE)

> saveRDS(natality_csv, here::here("dataset-ignore", 
+     "natality_rds.rds"))

> natality_rds = readRDS(here::here("dataset-ignore", 
+     "natality_rds.rds"))

> columns_to_remove <- c("mage_impflg", "lmpused", "obgest_flg", 
+     "compgst_imp", "imp_sex", "fagerpt_flg", "imp_plur", "mar_p", 
+     "fhisp_r" .... [TRUNCATED] 

> natality_data_cleaned <- select(natality_rds, -all_of(columns_to_remove))

> number_of_nas <- pivot_longer(summarize(natality_data_cleaned, 
+     across(everything(), ~sum(is.na(.)))), everything(), names_to = "Column", 
+   .... [TRUNCATED] 

> renamed_cols = c("dob_tt", "dob_mm")

> natality_data_cleaned <- select(mutate(mutate(natality_data_cleaned, 
+     birth_month = factor(month.name[dob_mm], levels = month.name)), 
+     t .... [TRUNCATED] 

> write_csv(natality_data_cleaned, file = here::here("dataset-ignore", 
+     "natality_data_cleaned.csv"))
